...
bool processVkShader(const std::string &assetPath, const std::string &outputPath,
  ...
  if (ext != ".hlsl") {
    ...
    auto glslPath = std::filesystem::path(outputPath).replace_extension("glsl");
    SirEngine::writeTextFileToDisk(glslPath.string(), res);
  }
