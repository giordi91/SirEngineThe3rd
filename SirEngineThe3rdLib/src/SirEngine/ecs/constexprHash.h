...
{
static constexpr unsigned int crc_table[256] = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419, 0x706af48f,
...
{
    template<class TT> 
    static constexpr const char* fnType(){return GENERATOR_PRETTY_FUNCTION;}
    static constexpr size_t hash = compile_hash::hash_fn<strl(fnType<T>())>(fnType<T>());
};
...
struct MultiHash
{
    ...
    static constexpr const char* fnType(){return GENERATOR_PRETTY_FUNCTION;}
    static constexpr size_t hash = hash_combine(compile_hash::hash_fn<strl(fnType<T>())>(fnType<T>()), MultiHash<Types...>::hash);
};