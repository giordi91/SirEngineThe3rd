...
#define ASSERT_CALLBACK_AND_DISPATCH(e)                         \
  ...
  switch (umessage) {
    ...
    case WM_SIZE: {
      // the reason for this check is because the window call a resize
      // immediately before the user has time to set the callback to the window
      // if
      auto callback = windowsApplicationHandle->getEventCallback();
      if (callback != nullptr) {
...
WindowsWindow::WindowsWindow(const WindowProps &props) {
  ...
  if (props.startFullScreen) {
    // extract full size usable by the window
    RECT outRect;

    BOOL fResult = SystemParametersInfo(SPI_GETWORKAREA, 0, &outRect, 0);
    SE_CORE_INFO("Creating WindowsWindow with dimensions: {0}x{1}",
                 outRect.right, outRect.bottom);
    w = outRect.right;
    h = outRect.bottom;
    //we also need to override the engine info
  	globals::ENGINE_CONFIG->m_windowWidth = w;
  	globals::ENGINE_CONFIG->m_windowHeight= h;
  } else {
    SE_CORE_INFO("Creating WindowsWindow with dimensions: {0}x{1}", props.width,
                 props.height);
  }
  ...
  m_hwnd = CreateWindowEx(0, title, title, style, 0, 0, wr.right - wr.left,
...
void WindowsWindow::onResize(const uint32_t width, const uint32_t height) {
  m_data.width = width;
